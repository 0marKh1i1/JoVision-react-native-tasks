...
const Task31 = () => {
  ...
  const renderImage = ({ item, index }: { item: imageData, index: number }) => {
    ...
    return (
      <View style={styles.imgContainer}>
        ...
        <TouchableOpacity onPress={() => handleImgDelete(item.id,index)} style={styles.deleteBtn}>
          <Text style={styles.deleteBtnText}>Delete</Text>
        </TouchableOpacity>
        <TouchableOpacity onPress={() => handleImgRepeat(item, index)} style={styles.repeatBtn}>
          <Text style={styles.deleteBtnText}>Repeat</Text>
        </TouchableOpacity>
      </View>
    );
  }
  ...
  const handleImgRepeat = (item: imageData,index: number) => {
    if(item){
      const updatedList = [...imageList];
      updatedList.splice(index +1, 0, {...item ,id : `${item.id}_copy_${index}`})
      setImageList(updatedList);
    }
  }
